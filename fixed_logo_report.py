#!/usr/bin/env python3
"""
Fixed Enhanced NHL Report Generator with Team Logos and Player Avatars
Properly handles image formats from NHL API
"""
...
class FixedLogoReportGenerator:
    def __init__(self):
    ...
    def download_and_convert_image(self, image_url, name):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(f"✅ Image downloaded and converted for {name}")
                return png_buffer
            else:
    ...
    def create_logo_header(self, away_team, home_team):
        ...
        try:
            ...
            if away_logo_url:
                away_logo_buffer = self.download_and_convert_image(away_logo_url, away_team.get('abbrev', 'Away'))
            if home_logo_url:
                home_logo_buffer = self.download_and_convert_image(home_logo_url, home_team.get('abbrev', 'Home'))
    ...
    def generate_fixed_logo_report(self, game_id, output_filename=None):
        """Generate the fixed report with team logos"""
        print("🏒 GENERATING FIXED NHL REPORT WITH TEAM LOGOS 🏒")
        print("=" * 80)
        ...
        if not output_filename:
            ...
            output_filename = f"fixed_logo_nhl_report_{game_id}_{timestamp}.pdf"
        ...
        story.append(Paragraph("🏒 FIXED NHL GAME REPORT WITH TEAM LOGOS 🏒", self.title_style))
        ...
        story.append(Paragraph("This enhanced report includes:", self.normal_style))
        story.append(Paragraph("• Team logos downloaded and properly formatted from NHL API", self.normal_style))
        story.append(Paragraph("• Player avatar URLs available for future enhancement", self.normal_style))
        ...
        story.append(Paragraph("• Fixed image format compatibility issues", self.normal_style))
        
        # Build PDF
        doc.build(story)
        
        print(f"✅ Fixed logo NHL report generated: {output_filename}")
        return output_filename

def main():
    """Generate the fixed logo NHL report"""
    print("🏒 FIXED NHL REPORT GENERATOR WITH TEAM LOGOS 🏒")
    print("=" * 60)
    
    generator = FixedLogoReportGenerator()
    
    # Use the game ID from your example
    game_id = "2024030242"
    
    print(f"Generating fixed logo report for game {game_id}...")
    result = generator.generate_fixed_logo_report(game_id)
    
    if result:
        print(f"🎉 SUCCESS! Fixed logo NHL report created: {result}")
        print("\nThis fixed report includes:")
        print("  • Team logos downloaded and properly formatted from NHL API")
        print("  • Player avatar URLs available for future enhancement")
        ...
        print("  • Fixed image format compatibility issues")
        print("  • This is the MOST VISUALLY ENHANCED NHL analytics report!")
    else:
        print("❌ Failed to generate fixed logo report")
