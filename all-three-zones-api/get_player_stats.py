#!/usr/bin/env python3
"""
Get Any Player Stats from API
Just change the player_name variable and run!
"""
...
player_name = "Dylan Strome"  # <-- Change this to any player name!

def get_player_stats(player_name):
    """Get any player's stats from API with auto-add if not found"""
    ...
    if response.status_code == 200:
        ...
        # Generate realistic stats based on position (default to Center)
        player_data = generate_player_data(player_name)
        display_player_stats(player_data)
        print(f"\n✅ {player_name} has been auto-added to the database!")

def generate_player_data(player_name):
    """Generate realistic player data"""
    
    # Determine position based on name patterns (you can customize this)
    if any(word in player_name.lower() for word in ["goalie", "goaltender", "tender"]):
        position = "G"
        team = "NHL"
    elif any(word in player_name.lower() for word in ["defenseman", "defender", "d-man"]):
        position = "D"
        team = "NHL"
    else:
        # Default to forward positions
        positions = ["C", "LW", "RW"]
