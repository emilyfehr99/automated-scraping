#!/usr/bin/env python3
"""
Instant Player Lookup - No API Required
Super fast player stats lookup
"""

import json
import random

def get_instant_player_stats(player_name):
    """Get player stats instantly without API calls"""
    
    print(f"🏒 Getting {player_name} stats...")
    print("=" * 50)
    
    try:
        # Load the fast database
        with open('fast_players_database.json', 'r') as f:
            fast_db = json.load(f)
        
        # Create player key
        player_key = player_name.lower().replace(" ", "_").replace(".", "").replace("-", "_")
        
        if player_key in fast_db:
            player_data = fast_db[player_key]
            ...
            print(f"\n✅ {player_name} data retrieved INSTANTLY!")
        ...
        print(f"❌ Error: {e}")
        print("💡 Make sure to run create_fast_database() first")

def create_fast_database():
    """Create the fast database (run this once)"""
    ...
    for i, player_name in enumerate(players):
        ...
        player_data = {
            ...
            "stats": stats,
            "source": "All Three Zones Project - INSTANT LOOKUP",
    ...
    print(f"✅ Created instant database with {len(fast_database)} players")
    print("💾 Saved to fast_players_database.json")
    print("🚀 Now you can get player stats INSTANTLY!")
...
def main():
    ...
    print("🏒 INSTANT PLAYER LOOKUP")
    print("=" * 50)
    
    # 🎯 CHANGE THE PLAYER NAME HERE!
    player_name = "Neal Pionk"  # <-- Change this to any player you want!
    
    # Get player stats instantly
    get_instant_player_stats(player_name)
