#!/usr/bin/env python3
"""
Enhanced NHL Report Generator with Team Logos and Player Avatars
Based on NHL API data that includes logo URLs
"""
...
class EnhancedLogoReportGenerator:
    def __init__(self):
    ...
    def create_logo_header(self, away_team, home_team):
        ...
        try:
            ...
            if away_logo_url:
                away_logo_buffer = self.download_logo(away_logo_url, away_team.get('abbrev', 'Away'))
            if home_logo_url:
                home_logo_buffer = self.download_logo(home_logo_url, home_team.get('abbrev', 'Home'))
    ...
    def generate_enhanced_logo_report(self, game_id, output_filename=None):
        """Generate the enhanced report with team logos"""
        print("🏒 GENERATING ENHANCED NHL REPORT WITH TEAM LOGOS 🏒")
        print("=" * 80)
        ...
        if not output_filename:
            ...
            output_filename = f"enhanced_logo_nhl_report_{game_id}_{timestamp}.pdf"
        ...
        story.append(Paragraph("🏒 ENHANCED NHL GAME REPORT WITH TEAM LOGOS 🏒", self.title_style))
        ...
        story.append(Paragraph("• Team logos downloaded from NHL API", self.normal_style))
        ...
        print(f"✅ Enhanced logo NHL report generated: {output_filename}")
        return output_filename

def main():
    """Generate the enhanced logo NHL report"""
    print("🏒 ENHANCED NHL REPORT GENERATOR WITH TEAM LOGOS 🏒")
    print("=" * 60)
    
    generator = EnhancedLogoReportGenerator()
    
    # Use the game ID from your example
    game_id = "2024030242"
    
    print(f"Generating enhanced logo report for game {game_id}...")
    result = generator.generate_enhanced_logo_report(game_id)
    
    if result:
        print(f"🎉 SUCCESS! Enhanced logo NHL report created: {result}")
        print("\nThis enhanced report includes:")
        print("  • Team logos downloaded from NHL API")
        ...
        print("❌ Failed to generate enhanced logo report")
