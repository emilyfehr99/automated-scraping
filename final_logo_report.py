#!/usr/bin/env python3
"""
Final NHL Report Generator with Team Logos and Player Avatars
Uses fallback approach for SVG logos and includes player headshots
"""
...
class FinalLogoReportGenerator:
    def __init__(self):
    ...
    def create_enhanced_header(self, away_team, home_team):
        """Create enhanced header with team information"""
        try:
            # Create header with team information (no logos for now)
            header_data = [
                ...
                [f"Team Logos: {away_team.get('logo', 'N/A')} | {home_team.get('logo', 'N/A')}"]
            ]
            ...
            print(f"Error creating enhanced header: {e}")
    ...
    def generate_final_logo_report(self, game_id, output_filename=None):
        """Generate the final report with team logos and player avatars"""
        print("🏒 GENERATING FINAL NHL REPORT WITH TEAM LOGOS & PLAYER AVATARS 🏒")
        print("=" * 80)
        ...
        if not output_filename:
            ...
            output_filename = f"final_logo_nhl_report_{game_id}_{timestamp}.pdf"
        ...
        story.append(Paragraph("🏒 FINAL NHL GAME REPORT WITH TEAM LOGOS & PLAYER AVATARS 🏒", self.title_style))
        story.append(Spacer(1, 10))
        
        # Create enhanced header with team information
        away_team = analysis_data['away_team']
        home_team = analysis_data['home_team']
        enhanced_header = self.create_enhanced_header(away_team, home_team)
        story.append(enhanced_header)
        ...
        story.append(Paragraph("This final report includes:", self.normal_style))
        story.append(Paragraph("• Team logo URLs from NHL API (SVG format available)", self.normal_style))
        ...
        story.append(Paragraph("• Professional layout with visual player branding", self.normal_style))
        ...
        print(f"✅ Final logo NHL report generated: {output_filename}")
        return output_filename

def main():
    """Generate the final logo NHL report"""
    print("🏒 FINAL NHL REPORT GENERATOR WITH TEAM LOGOS & PLAYER AVATARS 🏒")
    print("=" * 60)
    
    generator = FinalLogoReportGenerator()
    
    # Use the game ID from your example
    game_id = "2024030242"
    
    print(f"Generating final logo report for game {game_id}...")
    result = generator.generate_final_logo_report(game_id)
    
    if result:
        print(f"🎉 SUCCESS! Final logo NHL report created: {result}")
        print("\nThis final report includes:")
        print("  • Team logo URLs from NHL API (SVG format available)")
        print("  • Player headshots (PNG format) downloaded and displayed")
        print("  • All comprehensive metrics from previous reports")
        print("  • Professional layout with visual player branding")
        ...
        print("❌ Failed to generate final logo report")
