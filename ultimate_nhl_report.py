#!/usr/bin/env python3
"""
Ultimate NHL Report Generator with Player Stats and xG Model
Combines all analytics features into one comprehensive report
"""
...
class UltimateNHLReportGenerator:
    def __init__(self):
    ...
    def get_player_name_from_id(self, player_id, play_by_play_data):
        """Get player name from player ID using roster data"""
        if 'rosterSpots' in play_by_play_data:
            ...
            for spot in roster_spots:
                if spot.get('playerId') == player_id:
                    return spot.get('playerName', f'Player {player_id}')
        return f'Player {player_id}'
    
    def extract_comprehensive_statistics(self, play_by_play_data):
        ...
        for player_id, stats in player_stats.items():
            stats['name'] = self.get_player_name_from_id(player_id, play_by_play_data)
    ...
    def create_comprehensive_charts(self, analysis_data):
        ...
        try:
            ...
            if period_stats:
                ...
                bars = ax1.bar([f'Period {p}' for p in periods], goals_by_period, 
    ...
    def generate_ultimate_report(self, game_id, output_filename=None):
        """Generate the ultimate comprehensive NHL report"""
        print("🏒 GENERATING ULTIMATE NHL REPORT WITH PLAYER STATS & xG MODEL 🏒")
        print("=" * 80)
        ...
        analysis_data = self.extract_comprehensive_statistics(play_by_play_data)
        ...
        if not output_filename:
            ...
            output_filename = f"ultimate_nhl_report_{game_id}_{timestamp}.pdf"
        ...
        # Title
        story.append(Paragraph("🏒 ULTIMATE NHL GAME REPORT 🏒", self.title_style))
        story.append(Paragraph("Complete Analysis with Player Stats & Expected Goals Model", self.subtitle_style))
        ...
        # Top players table
        story.append(Paragraph("Top Players Performance", self.subtitle_style))
        ...
        player_table = Table(player_data, colWidths=[2*inch, 0.8*inch, 0.8*inch, 0.8*inch, 0.8*inch, 0.8*inch, 0.8*inch])
        ...
        story.append(Paragraph("This ultimate report includes:", self.normal_style))
        story.append(Paragraph("• Individual player statistics extracted from play-by-play data", self.normal_style))
        ...
        print(f"✅ Ultimate NHL report generated: {output_filename}")
        return output_filename

def main():
    """Generate the ultimate NHL report"""
    print("🏒 ULTIMATE NHL REPORT GENERATOR 🏒")
    print("=" * 50)
    
    generator = UltimateNHLReportGenerator()
    
    # Use the game ID from your example
    game_id = "2024030242"
    
    print(f"Generating ultimate comprehensive report for game {game_id}...")
    result = generator.generate_ultimate_report(game_id)
    
    if result:
        print(f"🎉 SUCCESS! Ultimate NHL report created: {result}")
        print("\nThis ultimate report contains:")
        print("  • Individual player statistics from play-by-play data")
        ...
        print("  • This is the ULTIMATE comprehensive NHL analytics report!")
    else:
        print("❌ Failed to generate ultimate report")
